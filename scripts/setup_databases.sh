...
# --- locating the vendored upstream download scripts -------------------------
# AlphaPulldown vendors AlphaFold 2 and 3, each with its own downloader. Use them
# rather than reimplementing URLs that upstream changes without notice.
find_upstream() {
  local relative=$1 root
  for root in "${ALPHAPULLDOWN_SRC:-}" "$PWD" "$PWD/.." "$HOME/AlphaPulldown"; do
    [[ -n "$root" && -f "$root/$relative" ]] && { printf '%s\n' "$root/$relative"; return 0; }
  done
  return 1
}
...
Point ALPHAPULLDOWN_SRC at an AlphaPulldown checkout, or download AlphaFold 2
databases following https://github.com/google-deepmind/alphafold"
...
Point ALPHAPULLDOWN_SRC at an AlphaPulldown checkout, or download AlphaFold 3
databases following https://github.com/google-deepmind/alphafold3"
